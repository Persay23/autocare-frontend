...
export default function Timeline() {
  ...
  const openExpense = useExpenseModal((s) => s.openFor)
  ...
  const handleEventClick = (event: TimelineEvent) => {
    ...
    if (event.type === 'Expense') {
      openExpense(event.relatedId)
    } else if (event.vehicleId && (event.type === 'Maintenance' || event.type === 'Service')) {
      openRecord(String(event.vehicleId), event.relatedId)
    } else if (event.vehicleId && (event.type === 'Fuel' || event.type === 'Liquid')) {
