...
export default function ComponentModal({ componentId, vehicleId, onClose, onSaved, onDeleted }: Props) {
  ...
  const computed  = c ? computeComponentMeasurements(c) : null
  const kmPercent    = computed?.kmPercent    ?? 100
  const yearsPercent = computed?.yearsPercent ?? 100
  const healthPct    = computed?.healthPct    ?? 100
  ...
  const kmColor      = colorFromPct(kmPercent)
  const yearsColor   = colorFromPct(yearsPercent)
  const aiHealthColor = c?.aiHealthPercent != null ? colorFromPct(c.aiHealthPercent) : 'var(--text)'
  const kmUsed         = c ? Math.max(0, (c.vehicleCurrentMileage ?? 0) - (c.installedAtVehicleMileage ?? 0)) : 0
  const remainingKm    = c ? Math.round((c.expectedLifetimeKm ?? 0) * kmPercent / 100) : 0
  const remainingYears = c ? (c.expectedLifetimeYears ?? 0) * yearsPercent / 100 : 0
                    ...
                    {history.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => { onClose(); openRecord(vehicleId, item.maintenanceRecordId) }}
                          style={{
