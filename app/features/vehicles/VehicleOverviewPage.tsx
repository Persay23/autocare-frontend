...
export default function VehicleOverviewPage() {
  ...
  const criticalCount = attnComponents.filter((c) => c.currentState === 'Critical').length
  ...
  return (
    <div>
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {attnComponents.slice(0, 4).map((c) => {
              const isCritical = c.currentState === 'Critical'
              return (
                <div
                  key={c.componentId}
                  onClick={(e) => { e.stopPropagation(); navigate(`/vehicles/${vehicleId}/components/${c.componentId}`) }}
                  style={{
      ...
      {/* Diagnose banner — mobile only (desktop uses sidebar Quick Actions) */}
      {!isDesktop && (
        <div
          onClick={() => vehicle && openFor(vehicle.vehicleId)}
          style={{
            margin: '0 22px 12px',
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--accent)' }}>Something Wrong?</div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: 'var(--text2)', marginTop: 2 }}>
              Describe symptoms · AI diagnosis
            </div>
          </div>
